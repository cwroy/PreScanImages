function twix_obj = mapVBVD_PreScan(filename,ScanFlag,varargin)


if ~exist('ScanFlag','var')
    ScanFlag=false;
end
...
% 
% Christopher W. Roy        * Modified to read prescan body and surface
%                             coil images 15/08/2022
%
%
% Input:
% 
...
for s=1:NScans
    ...
    rstraj = [];
    if arg.bReadHeader
        [twix_obj{s}.hdr, rstraj] = read_twix_hdr_PreScan(fid);
    end

    % declare data objects:
    twix_obj{s}.image         = twix_map_obj_PreScan(arg,'image',filename,version,rstraj);
        twix_obj{s}.prescanbody = twix_map_obj_PreScan(arg,'prescanbody',filename,version,rstraj);
        twix_obj{s}.prescansurface = twix_map_obj_PreScan(arg,'prescansurface',filename,version,rstraj);

%                 twix_obj{s}.SYNCDATA = twix_map_obj_PreScan(arg,'SYNCDATA',filename,version,rstraj);

    twix_obj{s}.noise         = twix_map_obj_PreScan(arg,'noise',filename,version);
    twix_obj{s}.phasecor      = twix_map_obj_PreScan(arg,'phasecor',filename,version,rstraj);
    twix_obj{s}.phasestab     = twix_map_obj_PreScan(arg,'phasestab',filename,version,rstraj);
    twix_obj{s}.phasestabRef0 = twix_map_obj_PreScan(arg,'phasestab_ref0',filename,version,rstraj);
    twix_obj{s}.phasestabRef1 = twix_map_obj_PreScan(arg,'phasestab_ref1',filename,version,rstraj);
    twix_obj{s}.refscan       = twix_map_obj_PreScan(arg,'refscan',filename,version,rstraj);
    twix_obj{s}.refscanPC     = twix_map_obj_PreScan(arg,'refscan_phasecor',filename,version,rstraj);
    twix_obj{s}.refscanPS     = twix_map_obj_PreScan(arg,'refscan_phasestab',filename,version,rstraj);
    twix_obj{s}.refscanPSRef0 = twix_map_obj_PreScan(arg,'refscan_phasestab_ref0',filename,version,rstraj);
    twix_obj{s}.refscanPSRef1 = twix_map_obj_PreScan(arg,'refscan_phasestab_ref1',filename,version,rstraj);
    twix_obj{s}.RTfeedback    = twix_map_obj_PreScan(arg,'rtfeedback',filename,version,rstraj);
    twix_obj{s}.vop           = twix_map_obj_PreScan(arg,'vop',filename,version); % tx-array rf pulses
...
%     fprintf('Scan %d/%d, read all mdhs:\n', s, NScans )
    fprintf('Scan %d/%d, read all mdhs:', s, NScans )
...
%             tmpMdh.(f{1}) = mdh.(f{1})( isCurrScan, : );
%         end
%         twix_obj{s}.SYNCDATA.readMDH( tmpMdh, filePos(isCurrScan) );
%         
    ...
    for scan = { 'image', 'noise', 'phasecor', 'phasestab', ...
                 ...
                 'refscanPSRef1', 'RTfeedback', 'vop','prescanbody','prescansurface' }
        f = scan{1};
...
noImaScan = ( mask.MDH_ACQEND           | mask.MDH_RTFEEDBACK   | mask.MDH_HPFEEDBACK    ...
            ...
            | (mask.MDH_PATREFSCAN & ~mask.MDH_PATREFANDIMASCAN) );
noImaScan = mdh.ushUsedChannels==2;
